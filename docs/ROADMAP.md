...
| 欄位 | 內容 |
|------|------|
| **目標** | 在重要 infra 變更完成後，同步更新專案 **Single Source of Truth** `server.md`，避免僅改 `inwanding-infra` repo 而總狀態文件過期 |
| **完成條件** | ① `server.md` 已反映最新真實狀態（主機、compose 路徑、容器名、volume、tunnel 指向、備份策略）② 與 `docs/ROADMAP.md`、README、DEPLOY、RESTORE **核心敘述一致**（無矛盾）③ 已更新「目前架構 / 部署方式 / 備份與還原 / 下一步」④ 若有里程碑（如 P0-2 push、P1-1 首次 DB 備份、P1-4 部署演練），完成後 **14 日內**有對應修訂紀錄或章節日期 |
| **風險等級** | 低 |
| **是否需要批准** | 否（僅文件）；若 `server.md` 含非公開位址 / 憑證指引，發佈前由你過目 |
| **建議順序** | 13（建議於 P0-2、P1-1、P1-4、P2-6 等里程碑後執行；可重複執行） |
