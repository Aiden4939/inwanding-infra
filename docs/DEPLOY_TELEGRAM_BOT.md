...
| `TELEGRAM_OPS_DOCKER_ENABLED` | `OPS_DOCKER_ENABLED` | `telegram-bot` | 啟用 `docker_ps` / `tail_logs` |
...
compose 已為 `telegram-bot` 掛載 `/var/run/docker.sock:ro`，並映射上述 `OPS_*` 變數。

| action | 說明 | 依賴 |
|---|---|---|
| `check_health` | HTTP 健康檢查 | `OPS_HEALTH_URLS` |
| `docker_ps` | 容器狀態 | docker.sock + `OPS_DOCKER_ENABLED` |
| `tail_logs` | 容器最近 log | docker.sock + `OPS_ALLOWED_CONTAINERS` 白名單 |
| `disk_usage` | `df -h` | 無額外依賴 |

**注意：**

- 容器名須與遠端 `docker compose ps` 一致（`svc-*`，非本機 dev 的 `telegram-agent-bot`）。
- `OPS_HEALTH_URLS` 須為 bot 容器內可達的 URL（例：`http://api:3000/health`）。
- 關閉 docker 查詢可設 `TELEGRAM_OPS_DOCKER_ENABLED=false`（仍可用 `check_health` / `disk_usage`）。
- 需 `telegram-agent-bot` 含 ops Phase 1 的 image（`feat/ops-phase1-executor` 合併後）。
...
- **ops 查詢**需 docker.sock 掛載與 `TELEGRAM_OPS_*` 設定；`tail_logs` 僅限 `OPS_ALLOWED_CONTAINERS` 白名單內容器。
